...
class ParsingSystem:
    ...
    def apply(self, c, t):
        ...
        if t.startswith("L"): 
            ######## Left Reduce, Adding arc from s1 to s2, s1 is head
            c.removeSecondTopStack()
            c.addArc(s1, s2, t[2:-1])

        elif t.startswith("R"):
            ###### Right reduce, Add arc from s2 to s1, s2 is head
            c.removeTopStack()
            c.addArc(s2, s1, t[2:-1])
        else:
            ###### Shift the top buffer element to top of the stack
            c.shift()
